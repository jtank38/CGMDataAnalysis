...
class getFeatures():
    ...
    def fft(self):
        ...
        f1 = [abs(list(tcal.fft_coefficient(self.data[i, :], [
            {"coeff": 11, "attr": "real"}]))[0][1]) for i in range(len(self.data))]
        f2 = [abs(list(tcal.fft_coefficient(self.data[i, :], [
            {"coeff": 12, "attr": "real"}]))[0][1]) for i in range(len(self.data))]
        f3 = [abs(list(tcal.fft_coefficient(self.data[i, :], [
            {"coeff": 13, "attr": "real"}]))[0][1]) for i in range(len(self.data))]
        f4 = [abs(list(tcal.fft_coefficient(self.data[i, :], [
            {"coeff": 14, "attr": "real"}]))[0][1]) for i in range(len(self.data))]
        f5 = [abs(list(tcal.fft_coefficient(self.data[i, :], [
            {"coeff": 15, "attr": "real"}]))[0][1]) for i in range(len(self.data))]
    ...
    def movingStd(self):
        ...
        df1 = df.rolling(window=32).std().T
